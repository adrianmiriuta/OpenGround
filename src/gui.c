...
static void gui_add_button_smallfont(uint8_t x, uint8_t y, uint8_t w, uint8_t h, char *str, f_ptr_t cb);
...
static void gui_render_bottombar(void) {
  ...
  screen_puts_xy(1, LCD_HEIGHT - 6, 0, __REVISION__);
...
void gui_render(void) {
  ...
  gui_touch_callback_register(0, GUI_PREV_CLICK_X, 0, LCD_HEIGHT, &gui_cb_previous_page);
  gui_touch_callback_register(LCD_WIDTH-GUI_PREV_CLICK_X, LCD_WIDTH, 0, LCD_HEIGHT, &gui_cb_next_page);
...
static void gui_cb_render_option_timer(uint32_t UNUSED(x), uint32_t y) {
  ...
  // render timer value
  screen_put_time(LCD_WIDTH / 2 - screen_strlen("1234") / 2, y, 1, storage.model[storage.current_model].timer);
}
